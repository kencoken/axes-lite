...
for (key, value) in dataPaths.iteritems():
...
if not os.path.isdir(component_opts['paths']['cpuvisor-srv']):
    subprocess.call('git clone git@github.com:kencoken/cpuvisor-srv.git ' + component_opts['paths']['cpuvisor-srv'], shell=True)
...
if not os.path.isdir(component_opts['paths']['imsearch-tools']):
    subprocess.call('git clone git@github.com:kencoken/imsearch-tools.git ' + component_opts['paths']['imsearch-tools'], shell=True)

if not os.path.isdir(component_opts['paths']['limas']):
    subprocess.call('hg clone ssh://hg@bitbucket.org/alyr/limas ' + component_opts['paths']['limas'], shell=True)

if not os.path.isdir(component_opts['paths']['axes-home']):
    subprocess.call('git clone git@bitbucket.org:kevinmcguinness/axes-home.git ' + component_opts['paths']['axes-home'], shell=True)